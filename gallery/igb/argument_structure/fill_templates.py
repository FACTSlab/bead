...
def ordinal_word(n: int) -> str:
    """Convert number to Igbo ordinal word."""
    ordinals = {
        1: "nke mbụ", 2: "nke abụọ", 3: "nke atọ", 4: "nke anọ", 5: "nke ise",
        6: "nke isii", 7: "nke asaa", 8: "nke asatọ", 9: "nke itoolu", 10: "nke iri"
    }
    return ordinals.get(n, f"nke {n}")


def render_template_ibo(
    template_string: str,
    slot_fillers: dict[str, LexicalItem],
    template_slots: dict[str, Slot]
) -> str:
    """Render template with proper form selection and 'other' handling for Igbo.

    Rules:
    - Uses item.form if available, otherwise item.lemma
    - If noun appears 2x: use "ọzọ" (another)       
    - If noun appears 3+x: use ordinals "nke abụọ", "nke atọ", etc.
    """
   
    # Count total occurrences of each noun
    noun_total_counts = count_noun_occurrences(slot_fillers, template_slots)
    
    # Identify noun+det pairs in template order
    ...
    # Build noun+det pairs in template order
    for noun_slot in noun_slots_in_order:
        suffix = noun_slot[5:]  # "subj", "dobj", "pobj", "iobj", etc.
        det_slot = f'det_{suffix}'
        if det_slot in slot_fillers and noun_slot in slot_fillers:
            noun_det_pairs.append((noun_slot, det_slot))
        elif noun_slot in slot_fillers:
            # Noun without determiner
            noun_det_pairs.append((noun_slot, None))
    ...
    for noun_slot, det_slot in noun_det_pairs:
        ...
        # Get determiner surface form if present
        det_surface = ""
        if det_slot and det_slot in slot_fillers:
        ...
        if occurrence == 0:
            # First occurrence: use original noun (+ det if present)
            if det_surface:
                rendered = f"{noun_surface} {det_surface}"
            else:
                rendered = f"{noun_surface}"
        elif total_occurrences == 2:
            # Exactly 2 occurrences: use "ọzọ" (another)
            if det_surface:
                rendered = f"{noun_surface} ọzọ {det_surface}"
            else:
                rendered = f"{noun_surface} ọzọ"
        else:
            # 3+ occurrences: use Igbo ordinals
            ordinal = ordinal_word(occurrence + 1)
            if det_surface:
                rendered = f"{noun_surface} {ordinal} {det_surface}"
            else:
                rendered = f"{noun_surface} {ordinal}"

        # Replace noun+det pair or just noun in template
        if det_slot and det_slot in slot_fillers:
            # Replace {noun_slot} {det_slot} pattern
            pattern = f"{{{noun_slot}}} {{{det_slot}}}"
            result = result.replace(pattern, rendered, 1)
        else:
            # Replace just {noun_slot}
            pattern = f"{{{noun_slot}}}"
            result = result.replace(pattern, rendered, 1)
...
def main() -> None:
    ...
    for lex_config in config["resources"]["lexicons"]:
        ...
        if args.dry_run and lex_config["name"] == "verbs":
            ...
            logger.info(
                f"DRY RUN: Limited verbs to 10 lemmas ({len(lexicon.items)} forms)"
            )
    ...
    for i, template in enumerate(templates, 1):
        ...
        try:
            combos = list(
                strategy.generate_from_template(template=template, lexicons=lexicons, language_code="ibo"
                )
            )
