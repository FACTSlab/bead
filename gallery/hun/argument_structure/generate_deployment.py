...
def create_minimal_item_template() -> ItemTemplate:
    ...
    Since the 2AFC items are already fully rendered, only a minimal
...
def patch_local_preview(list_output_dir: Path) -> None:
    """Patch the generated standalone preview so it does not depend on JATOS.

    The current bead jsPsych generator still writes JATOS list-distribution code
    into the local build even when use_jatos=False. For a local preview, each
    directory already contains exactly one experiment list, so lists[0] is used
    directly and render the existing 2AFC option_a / option_b fields as buttons.
    The JATOS build is left unchanged.
    """
    experiment_js = list_output_dir / "js" / "experiment.js"
    if not experiment_js.exists():
        return

    source = experiment_js.read_text(encoding="utf-8")
    source = source.replace("distributor = new ListDistributor(distConfig, lists);", "distributor = null;")
    source = source.replace("const listIndex = await distributor.initialize();", "const listIndex = 0;")
    source = source.replace("const assignedList = distributor.getAssignedList();", "const assignedList = lists[0];")
    source = source.replace(
        "stimulus: item.rendered_elements.text || 'No text',",
        "stimulus: '<p><strong>Which sentence sounds more natural?</strong></p>',",
    )
    source = source.replace(
        "choices: item.rendered_elements.choices || ['Option A', 'Option B'],",
        "choices: [item.rendered_elements.option_a, item.rendered_elements.option_b],",
    )
    experiment_js.write_text(source, encoding="utf-8")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Number of lists to select (defaults to config.yaml)",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default=None,
        help="Output directory (defaults to config.yaml)",
    )
    parser.add_argument(
        "--no-jatos",
        action="store_true",
    ...
    requested_lists = args.n_lists if args.n_lists is not None else int(deployment_config.get("n_lists_to_deploy", 16))
    output_dir_name = args.output_dir or deployment_config.get("output_dir", "deployment")
    random_seed = int(deployment_config.get("random_seed", 42))
    ...
    n_lists = min(requested_lists, len(all_lists))
    ...
    for version_name, use_jatos in versions:
        ...
        output_dir = base_dir / output_dir_name / version_name
        ...
        for i, exp_list in track(
            ...
            try:
                ...
                if version_name == "local":
                    patch_local_preview(list_output_dir)
    ...
    if not args.no_jatos:
        ...
        jatos_dir = base_dir / output_dir_name / "jatos"
    ...
    table.add_row("Output directory:", f"[cyan]{base_dir / output_dir_name}[/cyan]")
    table.add_row(
    ...
    if not args.no_jatos:
        table.add_row("", "")
        table.add_row("JATOS packages:", f"[cyan]{base_dir / output_dir_name / 'jatos'}/*.jzip[/cyan]")
