...
var jsPsychSashForcedChoice = (function (jspsych) {
  'use strict';

  const info = {
    name: 'sash-forced-choice',
    parameters: {
  ...
  class SashForcedChoicePlugin {
    ...
    trial(display_element, trial) {
      ...
      // Create HTML
      let html = '<div class="sash-forced-choice-container">';

      if (trial.prompt) {
        html += `<div class="sash-forced-choice-prompt">${trial.prompt}</div>`;
      }

      html += '<div class="sash-forced-choice-alternatives">';

      // Left alternative
      html += `
        <div class="sash-card sash-alternative" data-index="${left_index}" data-position="left">
          <div class="sash-alternative-label">Option 1</div>
          <div class="sash-alternative-content">${trial.alternatives[left_index] || 'Alternative A'}</div>
          <button class="sash-button sash-choice-button" data-index="${left_index}" data-position="left">
            Select
          </button>
        </div>
      `;
      ...
      // Right alternative
      html += `
        <div class="sash-card sash-alternative" data-index="${right_index}" data-position="right">
          <div class="sash-alternative-label">Option 2</div>
          <div class="sash-alternative-content">${trial.alternatives[right_index] || 'Alternative B'}</div>
          <button class="sash-button sash-choice-button" data-index="${right_index}" data-position="right">
            Select
          </button>
        </div>
      `;
      ...
      const choice_buttons = display_element.querySelectorAll('.sash-choice-button');
      ...
      const select_choice = (index, position) => {
        ...
        // Visual feedback
        const alternative_cards = display_element.querySelectorAll('.sash-alternative');
        alternative_cards.forEach((card) => {
          card.classList.remove('selected');
        });
        const selected_card = display_element.querySelector(`[data-position="${position}"]`);
        if (selected_card && selected_card.classList.contains('sash-alternative')) {
  ...
  SashForcedChoicePlugin.info = info;

  return SashForcedChoicePlugin;
})(jsPsychModule);
