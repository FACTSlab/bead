...
var jsPsychSashRating = (function (jspsych) {
  'use strict';
  ...
  class SashRatingPlugin {
    ...
    trial(display_element, trial) {
      ...
      // Create HTML
      let html = '<div class="sash-rating-container">';

      if (trial.prompt !== null) {
        html += `<div class="sash-rating-prompt">${trial.prompt}</div>`;
      }

      html += '<div class="sash-rating-scale">';

      // Create rating buttons
      for (let i = trial.scale_min; i <= trial.scale_max; i++) {
        const label = trial.scale_labels[i] || i;
        html += `
          <div class="sash-rating-option">
            <button class="sash-rating-button" data-value="${i}">${i}</button>
            <div class="sash-rating-label">${label}</div>
          </div>
        `;
      }
      ...
      // Continue button
      html += `
        <div class="sash-rating-button-container">
          <button class="sash-button sash-continue-button" id="sash-rating-continue" disabled>
            ${trial.button_label}
          </button>
        </div>
      `;
      ...
      const rating_buttons = display_element.querySelectorAll('.sash-rating-button');
      ...
      const continue_button = display_element.querySelector('#sash-rating-continue');
  ...
  SashRatingPlugin.info = info;

  return SashRatingPlugin;
})(jsPsychModule);
